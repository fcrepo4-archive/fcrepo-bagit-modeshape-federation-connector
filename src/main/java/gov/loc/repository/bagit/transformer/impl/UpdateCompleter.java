...
public class UpdateCompleter extends LongRunningOperationBase implements
        Completer {
    ...
    public UpdateCompleter(final BagFactory bagFactory) {
    ...
    /*
     * Limit updates to the provided payload filepaths, i.e., only the manifest
     * entries of the
     * provided files will be updated.
     */
    public void setLimitUpdatePayloadFilepaths(
            final List<String> limitUpdateFiles) {
        this.limitUpdatePayloadFilepaths = limitUpdateFiles;
    }

    /*
     * Limit deletes to the provided payload filepaths, i.e., only the manifest
     * entries of the
     * provided files will be removed.
     */
    public void setLimitDeletePayloadFilepaths(
            final List<String> limitDeleteFiles) {
        this.limitDeletePayloadFilepaths = limitDeleteFiles;
    }

    /*
     * Limit additions to the provided payload filepaths, i.e., only manifest
     * entries for the
     * provided files will be added.
     */
    public void setLimitAddPayloadFilepaths(final List<String> limitAddFiles) {
        this.limitAddPayloadFilepaths = limitAddFiles;
    }

    /*
     * Limit updates to the payload files in and below the provided directories,
     * i.e.,
     * only the manifest entries of files in or below the provided directories
     * will be updated.
     */
    public void setLimitUpdatePayloadDirectories(
            final List<String> limitUpdateDirectories) {
        this.limitUpdatePayloadDirectories = limitUpdateDirectories;
    }

    /*
     * Limit deletes to the payload files in and below the provided directories,
     * i.e.,
     * only the manifest entries of files in or below the provided directories
     * will be removed.
     */
    public void setLimitDeletePayloadDirectories(
            final List<String> limitDeleteDirectories) {
        this.limitDeletePayloadDirectories = limitDeleteDirectories;
    }

    /*
     * Limit additions to the payload files in and below the provided
     * directories, i.e.,
     * only manifest entries for files in or below the provided directories will
     * be added.
     */
    public void setLimitAddPayloadDirectories(
            final List<String> limitAddDirectories) {
        this.limitAddPayloadDirectories = limitAddDirectories;
    }

    /*
     * Limit updates to the provided tag filepaths, i.e., only the manifest
     * entries of the
     * provided files will be updated.
     */
    ...
    /*
     * Limit deletes to the provided tag filepaths, i.e., only the manifest
     * entries of the
     * provided files will be removed.
     */
    ...
    /*
     * Limit additions to the provided tag filepaths, i.e., only manifest
     * entries for the
     * provided files will be added.
     */
    ...
    /*
     * Limit updates to the tag files in and below the provided directories,
     * i.e.,
     * only the manifest entries of files in or below the provided directories
     * will be updated.
     */
    ...
    /*
     * Limit deletes to the tag files in and below the provided directories,
     * i.e.,
     * only the manifest entries of files in or below the provided directories
     * will be removed.
     */
    ...
    /*
     * Limit additions to the tag files in and below the provided directories,
     * i.e.,
     * only manifest entries for files in or below the provided directories will
     * be added.
     */
    ...
    protected void handleBagIt() {
        if (this.newBag.getBagItTxt() == null) {
            ...
            if (this.limitAddTagFilepaths != null) {
                this.limitAddTagFilepaths.add(this.newBag.getBagConstants()
                        .getBagItTxt());
            }
        }
    }
    ...
    protected void handleBagInfo() {
        final BagInfoTxt bagInfo = this.newBag.getBagInfoTxt();
        if (bagInfo == null) {
            //Do nothing
            return;
        }
        ...
        if (this.limitUpdateTagFilepaths != null) {
            this.limitUpdateTagFilepaths.add(this.newBag.getBagConstants()
                    .getBagInfoTxt());
        }
    ...
    protected void handleTagManifests() {
        if (this.newBag.getTagManifests().size() > 0) {
            ...
            this.helper.cleanManifests(this.newBag, this.newBag
                    .getTagManifests(), this.limitDeleteTagFilepaths,
                    this.limitDeleteTagDirectories);
            //Takes care of changed files
            for (final Manifest manifest : this.newBag.getTagManifests()) {
                this.helper.regenerateManifest(this.newBag, manifest, false,
                        this.limitUpdateTagFilepaths,
                        this.limitUpdateTagDirectories);
            }
            //Look for any added 
            ...
            this.helper.handleManifest(this.newBag, algorithm, ManifestHelper
                    .getTagManifestFilename(algorithm, this.newBag
                            .getBagConstants()), this.newBag.getTags(),
                    this.nonDefaultManifestSeparator,
                    this.limitAddTagFilepaths, this.limitAddTagDirectories);
        }
    }
    ...
    protected void handlePayloadManifests() {
        //Takes care of deleted files
        this.helper.cleanManifests(this.newBag, this.newBag
                .getPayloadManifests(), this.limitDeletePayloadFilepaths,
                this.limitDeletePayloadDirectories);
        //Takes care of changed files
        for (final Manifest manifest : this.newBag.getPayloadManifests()) {
            this.helper.regenerateManifest(this.newBag, manifest, false,
                    this.limitUpdatePayloadFilepaths,
                    this.limitUpdatePayloadDirectories);
        }
        //Looks for any added
        ...
        if (algorithm == null) {
            if (this.newBag.getPayloadManifests().size() > 0) {
                algorithm =
                        this.newBag.getPayloadManifests().get(0).getAlgorithm();
            } else {
                algorithm = Algorithm.MD5;
            }
        }
        this.helper.handleManifest(this.newBag, algorithm, ManifestHelper
                .getPayloadManifestFilename(algorithm, this.newBag
                        .getBagConstants()), this.newBag.getPayload(),
                this.nonDefaultManifestSeparator,
                this.limitAddPayloadFilepaths, this.limitAddPayloadDirectories);
