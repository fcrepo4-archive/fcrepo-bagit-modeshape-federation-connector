 ...
 * 
 */
public class ParallelManifestChecksumVerifier extends LongRunningOperationBase
        implements ManifestChecksumVerifier, FailModeSupporting {

    private static final Logger log = LoggerFactory
            .getLogger(ParallelManifestChecksumVerifier.class);
    ...
    public SimpleResult verify(final List<Manifest> manifests, final Bag bag) {
        ...
        log.debug(MessageFormat.format("Verifying manifests on {0} threads.",
                this.numberOfThreads));

        final SimpleResult finalResult = new SimpleResult(true);
        ...
        for (final Manifest manifest : manifests) {
            ...
            manifestCount++;
            this.progress("verifying manifest checksums", manifest
                    .getFilepath(), manifestCount, manifestTotal);

            final Manifest.Algorithm alg = manifest.getAlgorithm();
            final Iterator<String> manifestIterator =
                    manifest.keySet().iterator();
            final ArrayList<Future<SimpleResult>> futures =
                    new ArrayList<Future<SimpleResult>>(this.numberOfThreads);
            ...
            try {
                ...
                for (int i = 0; i < this.numberOfThreads; i++) {
                    final Future<SimpleResult> future =
                            threadPool.submit(new Callable<SimpleResult>() {
                                ...
                                public SimpleResult call() {
                                    ...
                                    for (final String filePath : safeIterator) {
                                        ...
                                        progress("verifying file checksum",
                                                filePath, fileCount
                                                        .incrementAndGet(),
                                                fileTotal);
                                        if (log.isDebugEnabled()) {
                                            log.debug(MessageFormat
                                                    .format("Verifying {1} fixity for file: {0}",
                                                            filePath,
                                                            alg.bagItAlgorithm));
                                        }
                                        ...
                                        if (file != null && file.exists()) {
                                            final String fixity =
                                                    manifest.get(filePath);
                                            InputStream stream = null;

                                            try {
                                                stream = file.newInputStream();
                                                if (!MessageDigestHelper
                                                        .fixityMatches(stream,
                                                                alg, fixity)) {
                                                    if (manifest
                                                            .isPayloadManifest()) {
                                                        SimpleResultHelper
                                                    ...
                                                    final String msg =
                                                            MessageFormat
                                                                    .format("Fixity failure in manifest {0}: {1}",
                                                                            manifest.getFilepath(),
                                                                            filePath);
                                                    log.debug(msg);
                                                    failFast.set(true);
                                                ...
                                                throw new RuntimeException(
                                                        MessageFormat
                                                                .format("Error checking fixity of {0}: {1}",
                                                                        filePath,
                                                                        ex.getMessage()),
                                                        ex);
                                            } finally {
                                            ...
                                            final String msg =
                                                    MessageFormat
                                                            .format("File missing from manifest {0}: {1}",
                                                                    manifest.getFilepath(),
                                                                    filePath);
                                            log.debug(msg);
                                            failFast.set(true);
                ...
                for (final Future<SimpleResult> future : futures) {
                    ...
                    try {
                        ...
                        futureResult =
                                new SimpleResult(false, e.getCause()
                                        .getMessage());
                        log.error(
                                "An error occurred while processing the manifest.",
                                e.getCause());
