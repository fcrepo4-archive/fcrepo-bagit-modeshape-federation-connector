...
public class ManifestImpl extends LinkedHashMap<String, String> implements
        Manifest {
    ...
    public ManifestImpl(final String name, final BagConstants bagConstants,
    ...
    public ManifestImpl(final String name, final BagConstants bagConstants,
        ...
        final ManifestReader reader =
                bagPartFactory.createManifestReader(sourceBagFile
                        .newInputStream(), bagConstants.getBagEncoding());
        try {
            while (reader.hasNext()) {
                final FilenameFixity filenameFixity = reader.next();
                this.put(filenameFixity.getFilename(), filenameFixity
                        .getFixityValue());
            }
        } finally {
    ...
    private void init(final String name, final BagConstants bagConstants,
            final BagPartFactory bagPartFactory) {
        log.debug("Creating manifest for " + name);
        this.name = name;
        this.bagConstants = bagConstants;
        this.bagPartFactory = bagPartFactory;
        if (!(ManifestHelper.isPayloadManifest(name, bagConstants) || ManifestHelper
                .isTagManifest(name, bagConstants))) {
    ...
    private InputStream generatedInputStream() {
        ...
        final ManifestWriter writer =
                this.bagPartFactory.createManifestWriter(out,
                        this.nonDefaultManifestSeparator);
        try {
            for (final String filename : this.keySet()) {
