...
public class FileDirNode extends AbstractFileNode implements DirNode {
    ...
    protected FileDirNode(final File file, final FileFileSystem fileSystem) {
    ...
    private synchronized void lazyInit() {
        ...
        for (final File child : FileHelper.normalizeForm(this.file.listFiles())) {
            ...
            if (child.isDirectory()) {
                ...
                throw new RuntimeException(MessageFormat.format(
                        "{0} is not a file or directory.", child));
            }
            this.childrenMap.put(childNode.getName(), childNode);
        }

        this.children =
                Collections.unmodifiableCollection(this.childrenMap.values());
    }
