...
public class ManifestMonitorIT {
    ...
    public void testEventsReceived() throws Exception {
        // Now create a listener implementation that will be called for our bag
        // created event..
        CountDownLatch addlatch = new CountDownLatch(1);
        EventLogger addlogger = getEventLogger(addlatch, Event.NODE_ADDED);
        CountDownLatch updateLatch = new CountDownLatch(1);
        EventLogger updateLogger =
                getEventLogger(updateLatch, Event.PROPERTY_CHANGED);
        ...
        verify(addlogger, times(1)).log(Mockito.eq(Event.NODE_ADDED),
        ...
        verify(updateLogger, times(1)).log(Mockito.eq(Event.PROPERTY_CHANGED),
                Mockito.startsWith("/objects/randomBag0"));
        ...
        verify(dellogger, Mockito.atLeastOnce()).log(
                Mockito.eq(Event.NODE_REMOVED),
                Mockito.eq("/objects/randomBag0"));
    }
     ...
     * @return
     */
    private EventLogger getEventLogger(CountDownLatch latch, int eventTypes)
        throws RepositoryException {
        ObservationManager observationManager =
                this.listenerSession.getWorkspace().getObservationManager();
        boolean isDeep = true; // if outputPath is ancestor of the sequencer
                               // output, false if identical
        String[] uuids = null; // Don't care about UUIDs of nodes for sequencing
                               // events
        String[] nodeTypes = null; // Don't care about node types of output
                                   // nodes for sequencing events
        boolean noLocal = false; // We do want events for sequencing happen
                                 // locally (as well as remotely)
        EventLogger result =
                Mockito.mock(LoggingEventListener.EventLogger.class);
        LoggingEventListener addlistener =
                new LoggingEventListener(latch, result);
        String outputPath = "/objects";
        observationManager.addEventListener(addlistener, eventTypes,
                outputPath, isDeep, uuids, nodeTypes, noLocal);
        return result;
    }
